...
_HOME_MAP_MARKERS = [
    ...
    # 恒生这一组三方挤在一起，dx/dy 是按 zoom=2（最窄的情况，宽屏只会更松）
    # 算出来的：香港圆点 py=180、上海 py=151、新加坡 py=241，标签盒 68×38。
    #   dy 必须是 0 —— 之前给了 10，标签下沿落到 226，正好压在新加坡圆点
    #   (241) 上方十几像素，看过去像"恒生"标的是新加坡那个点。归零之后标签
    #   紧贴香港自己的圆点（上沿 188，圆点 180，差 8），离新加坡圆点 15 像素。
    #   dx=-44 是为了跟上证的标签错开：上证 dx=+10 占 x[784,852]，
    #   恒生占 x[709,777]，留 7 像素缝。
    ("恒生指数", "HK", 22.2847, 114.1583, -44, 0),      # 港交所 中环交易广场
    ("上证指数", "A", 31.2222, 121.5083, 10, 0),         # 上交所 浦东南路528号
