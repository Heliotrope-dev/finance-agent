...
# 2026-09-02用户明确要求"起始值设成一万美金重新来过"，把2026-09-01那次
# "十万港币"的设定换掉——同样是代码层面的硬预算，机制不变（见下面原有
# 说明），只是数字换成一万美金折算成港币。汇率没有偷懒用sim_trader.py
# 那个7.80固定常量，是特意从data_sources.get_fx_rate实时查了一遍USD/CNY
# 和HKD/CNY交叉算出USD/HKD≈7.8384，跟固定常量7.80几乎一致（HKD本来就是
# 联系汇率钉住美元），所以仍然用7.80这个整数做最终换算，不为了"更精确"
# 引入一个以后每次读都可能不一样的浮点数——78,000 = 10,000美金 × 7.80。
_VIRTUAL_BUDGET_HKD = 78_000
...
# 股，不会自己冒出跨资产的ETF，所以单独给一份固定的跨资产ETF清单，每次
# 都无条件塞进候选池，不受"今天热不热门"影响。选的都是US/HK两个市场里
# 流动性最好、最不容易挂单挂不出去的头部ETF，不追求覆盖全，够AI做基础的
# 大类资产配置就行：
# - 美股债券：TLT(20年期以上美国国债)、IEF(7-10年期美国国债)——用长短
#   两个久期的国债ETF代表"债券"这类防御性资产。
# - 美股大宗商品：GLD(黄金)、USO(原油)——黄金是经典的避险/抗通胀资产，
#   原油代表能源类大宗商品，两者跟股票的相关性通常都不高。
# - 美股宽基指数：SPY(标普500)、QQQ(纳斯达克100)——跟"今天热门的某几支
#   个股"不是一回事，是分散到几百支股票的宽基敞口，波动天然比单一个股小。
# - 港股宽基：02800(盈富基金，追踪恒生指数)——港股这边流动性最好的宽基
#   ETF，同理提供分散敞口。
# 期货本身没有加入——Futu的期货交易走的是完全不同的账户体系
# (OpenFutureTradeContext，不是这里在用的OpenSecTradeContext)，接入期货
# 需要新开一整套期货模拟账户+完全独立的下单/持仓查询逻辑，跟"合理配置
# 不要全股票"这个诉求比，这次先用ETF覆盖债券/商品/宽基这几类最主要的
# 非个股资产，期货作为独立的后续工作量，不在这次改动范围内。
_DIVERSIFIER_ETFS: dict[str, list[tuple[str, str, str]]] = {
    "US": [
        ...
        ("USO", "原油ETF", "大宗商品"),
        ("DBC", "综合大宗商品ETF", "大宗商品"),
        # 2026-09-12用户要求"主要买股票期货大宗商品搭配买"，把商品这一类
        # 补齐到能真正做搭配的宽度：原来只有贵金属(GLD/SLV)+能源(USO)+综合
        # (DBC)，缺工业金属和农产品这两条跟经济周期关系完全不同的线——铜是
        # 典型的工业需求指标，农产品跟前面几类的驱动因素几乎不重叠。
        ("CPER", "铜ETF", "大宗商品"),
        ("DBA", "农产品ETF", "大宗商品"),
...
def _build_candidates(open_markets: list[str]) -> list[dict]:
    ...
    # 固定的跨资产ETF——不看当天热不热门，无条件塞进去，见_DIVERSIFIER_ETFS
    # 的说明。单个ETF查不到行情（停牌/富途暂时连不上）就跳过，不影响其它
    # 候选，也不影响个股那部分已经拿到的结果。
    for market in open_markets:
        for symbol, name, asset_class in _DIVERSIFIER_ETFS.get(market, []):
            try:
...
候选股当前行情（现价/涨跌幅/量比/换手率/52周区间位置/PE(TTM)/PB）、部分候选股会额外带一条
"投研顾问结论"、以及你自己过去几次决策——当时具体买卖了哪些标的、当时的判断理由、以及截至
...
- 它和你看到的盘面动能指向一致时（比如顾问给"买入"、今天又低位温和放量），是较强的确认，
  可以比平时更有底气一些；
- 指向相反时要格外谨慎——尤其"顾问观望/卖出但今天涨得猛"这种，往往正是情绪驱动的追高，
  是你过去反复吃亏的那一类；这种情况下除非你能明确说出盘面上有什么顾问看不到的新变化，
  否则不要买；
...
三是资产类别配置。用户2026-09-12进一步明确了偏好："主要买股票、期货、大宗商品搭配买"——
也就是说股票是主体，但要主动搭上大宗商品这一类，不要做成清一色股票的账户。期货这次没有
接入（原因不是遗漏：美股和新加坡期货这个账号没有行情权限，能取到行情的只有港股恒指期货，
而小型恒指一张合约名义价值约合3.2万美元，是这个一万美元账户的三倍多，一张就等于全仓押
单一方向，谈不上"搭配"），所以大宗商品这一类请用候选里带[大宗商品]标签的ETF来表达——
黄金/白银是贵金属，原油是能源，铜是工业需求，农产品是另一条驱动线，综合商品ETF是一篮子，
这几条线的驱动因素彼此不重叠，搭配起来才有意义，不是随便挑一个凑数。
用户此前也明确要求过"不要全部都是股票，债券期货大宗商品啥的ETF让他合理配置"——
候选清单里除了个股，还会看到几支带"[债券]"/"[大宗商品]"/"[宽基指数]"标签的ETF（比如
TLT/IEF这类国债ETF、GLD/USO这类大宗商品ETF、SPY/QQQ/02800这类宽基指数ETF），这些不是
凑数摆着的，是要求你真的纳入配置考虑的对象。个股天然波动大、彼此相关性也高（尤其同赛道的
更明显），账户长期只押注在个股上，等于把"选股能力"和"择时运气"这两件事的风险叠在了一起，
一旦判断错了缺乏缓冲。原则上，账户里非个股（债券/大宗商品/宽基指数ETF）的持仓市值合计
不应该长期是零，也不用规定死一个具体比例（不同市况下合理的比例本来就该浮动），但每一轮
决策看到当前持仓时，如果发现账户已经变成清一色个股、完全没有任何ETF敞口，就应该把"要不要
匀一部分仓位到ETF"当成这一轮认真考虑的选项之一，而不是习惯性地只在个股候选里挑，把ETF
候选直接跳过不看。这几支ETF同样要走下面第一步到第五步的完整流程（位置/估值/动能/止损/
仓位控制），不能因为"是ETF看着稳"就降低判断标准随手买，也不能因为"波动小不刺激"就
天然忽视它们。
...
真实教训(2026-09-03复盘)：不要把"52周区间处于低位"和"今天有没有已经涨过一大截"混为
一谈，这是两个完全不同的维度，"低位"说的是这支票在过去一年的相对位置，不代表"今天
现在这个价格"就便宜。如果一支候选股当天涨跌幅已经是两位数（比如当天涨了10%+），哪怕
量比很高、哪怕52周区间位置很低，本质上你要买的也是"追进一根当天已经走完的大阳线"，
不是低点介入——放量本身只能说明"今天确实有资金在动"，不能证明这个资金推动是真实利好
兑现还是纯情绪/资金炒作出货，尤其是PE为负（亏损）、市值小、你又没有新闻/公告渠道去
确认背后有没有真实催化剂的这类票，两位数的当天涨幅+放量组合更应该被当作"可能已经在
情绪尾部"的警示信号，而不是"低位放量"模板可以无脑套用的确认信号。遇到这种情况，要么
...
def _run_cycle_locked(email: str) -> dict:
    ...
    # 把"投研顾问"那条链路的结论接进来。这两条链路此前完全不通气：advisor.py
    # 每天17:30跑一次深度分析（财务摘要/新闻/研报/52周位置，几十次AI调用），
    # 而这里每5分钟跑一次，手里只有一眼行情。系统里已经算出来的那份更贵更厚的
    # 判断，高频决策一次都没用上——同一支票，顾问那边可能刚基于财报给了"观望"，
    # 这边却因为它今天涨得猛就买进去了。只取"结论+分数"这一点最精简的信息，
    # 不取长篇理由：候选池一次几十支，长文本会把本来就要75~110秒的AI调用拖得
    # 更慢，而边际价值很低。查不到的标的就是没有，不编。
    try:
    ...
    def _fmt_candidate(c: dict) -> str:
        ...
        if adv and adv.get("action"):
            ...
            parts.append(f"投研顾问结论：{adv['action']}{score_text}")
    ...
    try:
        ...
        # 交易时段、大家都在用的时候会经常卡在60秒这条线上超时。
        #
        # 100→200秒(2026-09-04)：100秒仍然不够，保留窗口里30轮有9轮
        # (30%)栽在这条线上。在VPS上用同规模prompt实测了三次真实延迟：
        # 98.9秒成功、100.5秒超时、75.0秒成功——也就是说真实耗时就在
        # 75~110秒之间徘徊，100秒这条线正好切在分布中间，一半概率被砍。
        # 顺带也验证过改流式能不能救：不能，实测首字节就要54/82/109秒，
        # 延迟全在模型侧(千问要把整段想完才吐第一个token)，真正的传输
        # 只占1~2秒，流式解决不了这个问题，白白引入拼接分片的复杂度。
        # 所以就是单纯放宽这条线。100秒这个值是15分钟节奏时代定的，那会儿
