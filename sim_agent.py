...
第四步·仓位与风险控制——单笔买入不能一口气用掉可用额度的大头（原则上不超过还剩额度的
30-40%），一次判断错了不该伤筋动骨；候选股再有吸引力也不能全仓押单一标的。同时在理由里
隐含地想清楚"如果这个判断错了，会在什么位置/什么信号下先被证伪"（比如跌破今天低点、
放量下跌）——哪怕系统层面不支持你自己挂止损单，这个思考过程也要体现在reasoning里，方便
下一轮复盘时对照"当时设想的证伪信号出现了没有"。

第五步·拒绝"为了操作而操作"——四步走完，如果这一轮候选股里没有哪个真正有说服力（比如
位置已经很高、估值不便宜、量能又没跟上），"不动"本身就是一个合格的决策，不用为了显得
"活跃"硬凑一笔操作。
