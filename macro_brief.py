...
_TOPICS = [
    ...
    (
        "china",
        "中国经济与政策",
        # 不能用裸 CPI/PPI/PMI：富途新闻会把美国同名数据排在前面，污染中国
        # 议题的 AI 背景材料。宏观序列本身已按 CN 区域取数，资讯也必须同样限定。
        ["中国CPI", "中国PPI", "中国制造业PMI", "社零", "中国消费", "M2", "货币供应", "央行", "LPR", "政治局会议"],
    ),
]
...
_TOPIC_SERIES = {
    ...
    # 会漏掉生产端通胀、消费强弱及信用环境，无法支持 沪深/港股的行业判断。
    "china": [
        ("CN", "中国CPI同比"),
        ("CN", "中国工业生产者出厂价格"),
        ("CN", "中国官方制造业采购经理人指数"),
